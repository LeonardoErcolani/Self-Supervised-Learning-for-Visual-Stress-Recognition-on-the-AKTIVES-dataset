...
def create_full_dinov2_temporal_features():
    """
    Create all temporal features for full frame DINOv2 features using shared utilities.
    This script creates single, triple, and overlapping interval features.
    """
    print("=== Creating Full Frame DINOv2 Temporal Features ===\n")
    
    # Define paths for full frame DINOv2 features
    per_second_path = "Processed Data/DinoV2 Features Extracted/all_features_per_second.csv"
    interval_path = "Processed Data/DinoV2 Features Extracted/all_features_per_interval.csv"
    ...
    print("Loading full frame DINOv2 data...")
    ...
    single_df = create_single_interval_features(
        per_second_df, interval_df, "feat_", single_output
    )
    print_dataset_statistics(single_df, "Full Frame DINOv2 Single Interval")
    ...
    triple_df = create_triple_interval_features(
        per_second_df, interval_df, "feat_", triple_output
    )
    print_dataset_statistics(triple_df, "Full Frame DINOv2 Triple Intervals")
    ...
    overlapping_df = create_overlapping_interval_features(
        per_second_df, interval_df, "feat_", overlapping_output
    )
    print_dataset_statistics(overlapping_df, "Full Frame DINOv2 Overlapping Intervals")
    
    print("\n✅ All full frame DINOv2 temporal features created successfully!")
...
if __name__ == "__main__":
    create_full_dinov2_temporal_features()
