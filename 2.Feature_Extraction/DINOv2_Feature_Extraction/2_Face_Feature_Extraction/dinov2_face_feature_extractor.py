...
class DINOv2FaceFeatureExtractor:
    def __init__(self):
...
def process_face_images_and_metadata():
    """Process face images and append features to matching metadata rows"""
    ...
    print("Initializing DINOv2 face feature extractor...")
    extractor = DINOv2FaceFeatureExtractor()
    
    # Create a list to store per-second features
    per_second_features = []
    
    # Process each interval and append features to matching metadata row
    print("Processing face images and extracting features...")
    for idx, row in tqdm(metadata_df.iterrows(), total=len(metadata_df)):
        ...
        # Construct face interval directory path
        interval_dir = os.path.join("Processed Data/Face Data", 
        ...
        if os.path.exists(interval_dir):
            ...
            if features_avg is not None:
                ...
                for i, feat in enumerate(features_avg):
                    metadata_df.at[idx, f'face_feat_{i}'] = feat
                ...
                for frame_idx, frame_features in enumerate(features_sequence):
                    per_second_row = {
                        ...
                        **{f'face_feat_{i}': feat for i, feat in enumerate(frame_features)}
                    }
    ...
    output_path = os.path.join(output_dir, "all_face_features_per_interval.csv")
    metadata_df.to_csv(output_path, index=False)
    print(f"Face averaged features saved to: {output_path}")
    
    # Save the per-second features
    per_second_df = pd.DataFrame(per_second_features)
    per_second_path = os.path.join(output_dir, "all_face_features_per_second.csv")
    per_second_df.to_csv(per_second_path, index=False)
    print(f"Face per-second features saved to: {per_second_path}")
    
    # Print summary statistics
    print(f"\n=== Face Feature Extraction Summary ===")
    print(f"Total intervals processed: {len(metadata_df)}")
    print(f"Total face frames processed: {len(per_second_df)}")
    print(f"Face feature dimensions: {metadata_df.filter(like='face_feat_').shape[1]}")
...
if __name__ == "__main__":
    process_face_images_and_metadata() 